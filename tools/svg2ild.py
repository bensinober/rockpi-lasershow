...
def write_ild(params, rframe, path, center=True):
	...
	samples = len(rframe)
	print("params: force: {} invert: {}, rate: {} time: {}+n".format(params.force,params.invert,params.rate,params.time))
	...
	for i,sample in enumerate(rframe):
		...
		# ILDA version 0: x, y, z (int16), status, colour idx (uint8)
		# ILDA version 1: x, y (int16), status, colour idx (uint8)
		dout += struct.pack(">hhBB",x,-y,mode,0x01 if sample.on else 0x00)
	...
	for i in range(frames):
		hdr = struct.pack(">4s3xB8s8sHHHBx", b"ILDA", ilda_format, b"svg2ilda", b"", samples, i, frames, 0)
		fout.write(hdr)
		fout.write(dout)

	# Write last header, 0 length
